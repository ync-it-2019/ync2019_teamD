...
<!--여기까지 바 그래프-->
	...
	<c:forEach items="${investorList}" var="investor">
		...
		<c:choose>
			<c:when test="${investor.job == '일반'}">
				<c:set var= "ssum" value="${ssum + 1}"/>
			</c:when>
		
			<c:when test="${investor.job == '공무원'.toString()}">
				<c:set var= "ssum2" value="${ssum2 + 1}"/>
			</c:when>
		
			<c:when test="${investor.job eq 'CEO'.toString()}">
				<c:set var= "ssum3" value="${ssum3 + 1}"/>
			</c:when>
		
			<c:when test="${investor.job eq '자영업'.toString()}">
				<c:set var= "ssum4" value="${ssum4 + 1}"/>
			</c:when>
		
			<c:when test="${investor.job eq '직업5'.toString()}">
...
<script type="text/javascript">
	...
	function drawChart() {
		var data = google.visualization.arrayToDataTable([
				[ 'Task', 'Hours per Day' ],
				[ '일반' + ' ' + <c:out value="${ssum}"/> + '명',<c:out value="${ssum}"/> ],
				[ '공무원' + ' ' + <c:out value="${ssum2}"/> + '명',<c:out value="${ssum2}"/> ],
				[ 'CEO' + ' ' + <c:out value="${ssum3}"/> + '명',<c:out value="${ssum3}"/> ],
				[ '자영업' + ' ' + <c:out value="${ssum4}"/> + '명',<c:out value="${ssum4}"/> ],
				[ '직업5' + ' ' + <c:out value="${ssum5}"/> + '명',<c:out value="${ssum5}"/> ] ]);
...
<link href="/resources/css/jiwon.css" rel="stylesheet" type="text/css"
	media="all" />
				...
				<div class="col-md-4-1" style="float: top; width: 100%; height:40%;">
					...
					<h3>
						<span class="item_price"> <!--날짜 계산  --> <jsp:useBean
								...
								var="nowDate"></fmt:parseNumber> ${endDate - nowDate} 일 남음<!--날짜 계산 end  -->
					...
					<p>
						<span class="item_price" style="font-size: 25px;"><fmt:formatNumber
								value="${project.current_invest}" pattern="#,###,###,###,###" /></span>원
						펀딩
					</p>
					<br>
					<p>
						<span style="font-size: 25px;"><c:out
								value="${investor_total}" /></span>명의 투자자
					</p>
					<br>
							...
							<div class="modal-content">
								<form class="" action="../invest/funding" method="post">
									...
									<div class="modal-body">
										...
										금액 입력 <br> <input type="text" class="form-control"
											placeholder="원하는 금액을 입력하세요." id="invest_content"
										...
										<input type="radio" name="score" value=1 checked> 수익 <input
											type="radio" name="score" value=2> 안정 <input
											type="radio" name="score" value=3> 성장 <input
											type="radio" name="score" value=4> 시장 <input
											type="radio" name="score" value=5> 호감<br>
									...
									<div class="modal-footer">
										...
										&nbsp;&nbsp; <input type="submit" class="btn btn-danger2"
											name="submit" value="투자하기" class="button">
							...
							<div style="border: 1px solid gray; margin: 10px; height: 200px;">
								<div style="height: 60%;">
									...
									<div style="float: left; width: 60%; ">
										<p>회사명 : <strong style="font-size: 15px;"><c:out value="${project.company}"/></strong></p>
										
										<p><c:out value="${project.self_introduce}"/></p>
									</div>
								</div>
								<div style="height: 40%;">
									<div id="flip" style="margin: 10px 0px 0px 0px;">작성자 연락처</div>
								</div>
			...
			<div class="responsive_tabs_agileits"
				...
				<div id="horizontalTab">
					...
					<div class="resp-tabs-container">
						...
						<div class="tab2">
							<div class="single_page_agile_its_w3ls">
								<p style="font-size:25px; color:black;">현재 이 프로젝트에</p>
								<p><strong style="font-size: 25px; color: #00CCCC;"><c:out value="${invest_total}"/></strong><span style="font-size:25px; color:black;">명의 참여가 이루어졌습니다.</span></p>
											...
											<div class="media p-3">
												...
												<div class="media-body">
													...
													<p style="font-size: 20px;">
														<strong style="font-size: 20px;"><c:out value="${investor.nickname}" /></strong>님이
														<strong style="font-size: 20px;"><fmt:formatNumber value="${investor.amount}" pattern="#,###,###,###,###" />원</strong> 투자로 참여 하셨습니다.
													</p>
												</div>
										</div>
										</c:forEach>
									...
									<div class="col-lg-12">
										...
										<div class="panel panel-default">
											...
											<div class="panel-heading">
												<i class="fa fa-ripples fa-fw"></i>GoreNani Ripple

												<sec:authorize access="isAnonymous()">
													<a href="#" onclick="myPageLogin()"
														class='btn btn-primary btn-xs pull-right'> 리플 작성 </a>
												</sec:authorize>
												<sec:authorize access="isAuthenticated()">
													<button id='addReplyBtn'
														class='btn btn-primary btn-xs pull-right'>리플 작성</button>
												</sec:authorize>
														...
														<p>
															...
															<sec:authorize access="isAuthenticated()">
																<c:if test="${princ eq ripple2.userid}">
																	<button class='modalModBtn btn btn-warning'type="button" onclick="modifying('${ripple2.ripple_num}','${project.project_num}','${ripple2.content}','${ripple2.userid}')">Modify</button> 
						...
						<c:if test="${not empty project.attach_1}">
							<p style="padding: 0 0 5px 0;">
								<input type="button" class="btn" onclick="location.href='/resources/upload/${project.attach_1}'" download value="상세정보.pptx"/>
							</p>
						</c:if>
						<c:if test="${not empty project.attach_2}">
							<p style="padding: 0 0 5px 0;">
								<input type="button" class="btn" onclick="location.href='/resources/upload/${project.attach_2}'" download value="상세정보2.pptx"/>
							</p>
						</c:if>
						<c:if test="${not empty project.attach_3}">
							<p style="padding: 0 0 5px 0;">
								<input type="button" class="btn" onclick="location.href='/resources/upload/${project.attach_3}'" download value="상세정보3.pptx"/>
							</p>
						</c:if>
					</div>
					<br>
								...
								<sec:authentication property="principal" var="pinfo"/>
									<sec:authorize access="isAuthenticated()">
										<c:if test="${pinfo.username eq project.userid}">
											<input type="submit" id="remove_btn" data-oper='remove' value="삭제하기" class="btn btn-danger">
										</c:if>
									</sec:authorize>
						</form>
				</div>
					...
					<div class="modal-dialog">
						...
						<div class="modal-content">
							<div class="modal-header">
								...
								<h4 class="modal-title" id="myModalLabel">댓글 수정 .GoreNani</h4>
							</div>
							<div class="modal-body">
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" name="userid" value="${login_user.userid}">--%>
								</div>
					...
					<div class="modal-dialog">
						...
						<div class="modal-content">
							<div class="modal-header">
								<button type="button" class="close" data-dismiss="modal"
									aria-hidden="true">&times;</button>
								<h4 class="modal-title" id="myModalLabel">댓글 등록 .GoreNani</h4>
							</div>
							<div class="modal-body">
								<div class="form-group">
									<label>Content</label>
